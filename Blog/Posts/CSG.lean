/-
Copyright (c) 2023-2024 Lean FRO LLC. All rights reserved.
Released under Apache 2.0 license as described in the file LICENSE.
Author: David Thrane Christiansen
-/

import VersoBlog
import Blog.Categories
import Mathlib.Data.Real.Basic
import Mathlib.Analysis.InnerProductSpace.PiL2
open Verso Genre Blog

#doc (Post) "Vibe coding with .lean instead of .md files" =>

%%%
authors := ["Tomas Skrivan"]
date := {year := 2026, month := 2, day := 20}
categories := []
%%%


I'm a computer graphics programmer, I mostly specialize on physics simulations or computational geometry processing problems. As a trained mathematician, I really enjoy thinking about the mathematics part but the coding part I always found as the necessary evil of my job. Roughly five years back, I discovered interactive proof assistans and realized that they could revolutionalize how I write code. I would start from a precise mathematical formulation of the problem and then interactivelly transform it to executable code. So I started working on Lean 4 libray SciLean which would hopefully bring this idea to life. After four years working on it I would say that the project is effectivelly dead at this point as it did not manage gain any meaningful traction and I ran out of energy. It wasn't addressing any of the problems current Lean users have and potential target audience would have very little motivation to switch to Lean ecosystem from vastly more mature languages like C++, Python or Julia.

I still believe that the core idea of writting mathematical specification and transforming it to executable code is sound. However, SciLean's approach was fundamentally flawed as it tried to provide fixed set of transformation rules. The design space for there rules is just too vast and implementig a pratically useful subset is effectivelly imposible.

The rise of LLMs asks for rethinking the fundamental approach behind SciLean. AI got so good that I belive that the workflow should look like this
1. hand write precise mathematical specification of your problem in Lean
2. transform the Lean specification into more detailed Lean specification/code while tieing back to the original spec with proofs
3. generate the code in you target language based on the detailed Lean specification

The way I think about it is that the at step one I specify what I really want to implement, but it is too crude for the AI to be able to just generate usable code from it. Therefore we need to transform it into more detailed specification. This is the step where you inject all the engeneering knowlege and practical world constrains. For example, the optimal code should have very different for whether you are targeting CPU, GPU or distributed systams. Think about it that the detailed specification is the specification you would have to write yourself in and `.md` files for the AI to have a chance at generating the code you want. Using Lean we can keep a tight connection between the top level specification and more detailed one.

The main motivation for working this way for me is that I can modify or extend the top level specification and get deterministic feedback on whether these changes break or not the core ideas in my code. Without using Lean I would have to manually go over all the `.md` files and check if the the change in what I'm building impacts the engeneering decisions.

Recently, at work I had to write bunch of tools working with signed distance functions. Their mathematical description is very simple so I thought that this might be a very good test for this workflow. This blog post is documenting my journey.

# Case Study: Constructive Solid Geometry

```leanInit post
```
```leanInit post'
```

```lean post
open EuclideanSpace Topology Metric NNReal
notation "ℝ^" n => EuclideanSpace ℝ (Fin n)
notation "ℝ≥0^" n => EuclideanSpace (ℝ≥0) (Fin n)
noncomputable section
open Classical
variable {n : ℕ}
```

Let's start with a bit of a background on signed distance function and constructive solid geometry.

Signed distance functions are a very usefull tool for representing solid objects. For an object $`\Omega` we define signed distance function $`\phi` as
$$`
\phi(x) = \begin{cases}
  -d(x, \partial \Omega) & \text{if } x \in \Omega \\
   d(x, \partial \Omega) & \text{if }\, x \notin \Omega\\
   0 & \text{if }\, x \in \partial \Omega.
\end{cases}
`
Many operations are very easy with signed distance functions such as collision detection. Determining if a point is inside of the object we just need to check that $`\phi(x) < 0` and projecting the point back to the surface of the objects is just $`x^* := x - \phi(x) \cdot \nabla \phi(x)`.

To give you a better idea about signed distance functions the following picture visualizes the signed distance function of a ball. Negative values are blue and positive are orange with black isoline every 0.2.

![polygon_vs_sdf](static/imgs/polygon_vs_sdf.png)

The picture puts the SDF representation in a contrast with polygonal representation of a ball(on the left) for which exact collision detection is way more difficult and computationally expensive. Furthermore, sphere can be only approximated with polygon. On the otherhand, with SFD representation a ball is just $`\phi(x) = \|x - c\| - r` where $`c` is the center and $`r` is the radius.

There are many basic shapes for which we can write down exact signed distance function. I higly recommend having a look at [Inigo Quilez's article](https://iquilezles.org/articles/distfunctions/) on 3D SDFs. More compilcated objects can be assembled from these basic shapes. For example, in video games complex polygonal model, like the one on the left, are approximated for the purpose of collision detection by a handfull of simple shapes, on the right with its SDF visualized as previously.

![compound_shape](static/imgs/polygon_vs_sdf.compound_shape.0001.png)


Creating more complex shapes out of simple ones is called [*Constructive Solid Geometry*](https://en.wikipedia.org/wiki/Constructive_solid_geometry)(CSG). Mathematically is just performing set operations such as union, intersection or complement. Which translates to $`\min`, $`\max` and negation on SDFs.

$$`
\begin{align*}
A \cup B &= \{ x | \min(\phi_A(x), \phi_B(x)) \le 0 \} \\
A \cap B &= \{ x | \max(\phi_A(x), \phi_B(x)) \le 0 \} \\
A^c &= \{ x | - \phi_A(x) ≤ 0 \}
\end{align*}
`

At my work, I had to write tools to work with these shapes. This roughly included:
- around 15 basic shapes
- CSG operations
- fast evaluation at many locations
- computing bounds on SDFs in a region
- pruning CSG expressions
- conversion to voxel representation
- computing derivatives in space and w.r.t. to parameters

It is worth mentioning that lots of inspiration this work on SDfs came from reading [Matt Keeter's articles](https://www.mattkeeter.com/).

All of these SDF operations have very simple mathematical specification but the implementation can get really tedious. As a mathematician at hearth, I really enjoy thinking about the math and algorithms but the coding is just the necessary evil of my job. This got me thinking, can I just write the specification in Lean and let AI generate the necessary C code?

The purpose of this blog post is to documents this experiment. To fully capture what went wrong and what went well I mostly wrote this article as I progressed through the experiment.


# Vibe coding based on Lean specification

The goal of this experiment is to vibe code a C library capable of doing the very basic constructive solid geometry operations. The envisioned process is to write down manually the most pure mathematical specification in Lean, then masage the specification into Lean code until it is in the form which can be translated to C by the AI with very little room for misinterpretation.

My experience with vibe coding is that it is quite hard to write clean and precise specification in English. My works is mostly in computational physics or computer graphics where a concise mathematical specification of the problem is often possible so I would rather write that than english prose.


The first step is to write down the formal definitin of the signed distance function:
```lean post
def sdf (s : Set (ℝ^n)) (x : ℝ^n) : ℝ :=
  let dist := infDist x (frontier s)
  if x ∈ s then - dist else dist
```

where `infDist` is the distance between a point and a set and `frontier` is the boundary of a set.

Keep in mind that this function is just a specification. Real numbers `ℝ` nor generic set `Set (ℝ^n)` don't have a direct executable representation. The functions `infDist`, `frontier` and `∈` are also just pure specifications and they can not be executed.

When turning this function to actual C implementation we will replace real numbers `ℝ` with floating point numbers but what do we do with a generric sets `Set (ℝ^n)`? Recall that we have explicit formulas for exact SDF only for handful basic shapes.

For simplicity of this experiment, let's just work with balls and boxes. Let's define a type representing a basic shape
```lean post
inductive BasicShape (n : ℕ) where
  | ball (center : ℝ^n) (radius : ℝ)
  | box (center : ℝ^n) (size : ℝ^n)
```
On C level we will want to represent this with a tagged union and it will be the data on leaf of the CSG tree.

What we need nest is interpretation of `BasicShape` as `Set (ℝ^n)` and explicit SDF formulas for them.

Because I already wrote to much code, let's as AI to do that for us

```
❯ Please define BasicShape.toSet turning basic shape to Set (ℝ^n) and
  BasicShape.sdf which provides an explicit furmla for signed distance function.
```

Which generated
```lean post
protected def BasicShape.toSet : BasicShape n → Set (ℝ^n)
  | .ball center radius =>
      Metric.closedBall center radius
  | .box center size    =>
      {x | ∀ i : Fin n, |x i - center i| ≤ size i / 2}

protected def BasicShape.sdf (x : ℝ^n) : BasicShape n → ℝ
  | .ball center radius => ‖x - center‖ - radius
  | .box center size =>
    let q : Fin n → ℝ :=
      fun i => |x i - center i| - size i / 2
    let outerDist :=
      Real.sqrt (∑ i : Fin n, (max (q i) 0) ^ 2)
    let maxQ : ℝ :=
      if h : (Finset.univ : Finset (Fin n)).Nonempty
      then Finset.sup' Finset.univ h q
      else 0
    outerDist + min maxQ 0
```

`BasicShape.toSet` looks good but `BasicShape.sdf` is getting too complicated for my slop coder degenerate brain. We need a guarantee that `BasicShape.sdf` is actually equal to `sdf`.

Let's ask AI to do that
```
❯ Can you please state that `sdf` is equal to `BasicShape.sdf` and try to prove it.
```

My hope was to get this theorem and its proof
```lean post
theorem BasicShape.sdf_eq (s : BasicShape n) (x : ℝ^n) :
    sdf s.toSet x = s.sdf x := sorry
```
but it didn't ... instead of that the AI burned through ~60k tokens and did not really produce any reasonable working code. On the other, it pointed out that it can't really state the theorem as such because it is not true for negative ball radius, or negative box size. This is actually a common source of bugs I have encountered. There are three ways to interpret ball with negative radius, it is just an empty set, it is a ball with that radius ignoring the negative sign, or a ball complement with that radius.

We have two options, either litter `BasicShape.toSet` and `BasicShape.sdf` with absolute values or forbit negative ball radii or negative box size. Let's try the second approach

```lean post
inductive BasicShape' (n : ℕ) where
  | ball (center : ℝ^n) (radius : ℝ≥0)
  | box (center : ℝ^n) (size : ℝ≥0^n)
```
Creating ball with negative radius will fail now
-- ```lean post (name:=negball)
-- #check BasicShape'.ball !₂[0,0] (-1)
-- ```
-- ```leanOutput negball
-- BasicShape.ball !₂[0, 0] (-1) : BasicShape 2
-- ```

Rather than burning my precisious tokens and asking general coding AI to prove that `sdf s.toSet x = s.sdf x` I thought this might be finally a good reason to finally try out [Harmonic's Aristotele](https://aristotle.harmonic.fun/). Sure enough, it is true! You can have a look at the proof [here][proof1]. Except in the case `n = 0` which is a case we do not case about anyway.

We are now done with basic shapes for now, lets turn our attention to the CSG operations. For simplicity let's consider only union, intersection and rigid transformation. This can be represented with this inductive type
```lean post
inductive CompoundShape (n : ℕ) where
  | basic     (s : BasicShape n)
  | transform (s : CompoundShape n)
              (A : Matrix.orthogonalGroup (Fin n) ℝ) (t : ℝ^n)
  | union     (a b : CompoundShape n)
  | intersect (a b : CompoundShape n)
```

Similar to `BasicShape` we want to have `toSet` and `sdf` functions. The only caveate that `min/max` do not preciselly conserve SDFs so we rather call is `csgSdf`.

```
❯ Please define `CompoundShape.toSet` and `CompoundShape.csgSdf` analogous to `BasicShape` functions. The `CompoundShape.csgSdf` is not a true SDF, it should turn union into `min` and intersection to `max` operation.
```


```lean post
def CompoundShape.toSet : CompoundShape n → Set (ℝ^n)
  | .basic s         => s.toSet
  | .transform s A t =>
    {y | WithLp.toLp 2 (A.val.transpose.mulVec (WithLp.ofLp (y - t))) ∈ s.toSet}
  | .union a b       => a.toSet ∪ b.toSet
  | .intersect a b   => a.toSet ∩ b.toSet

def CompoundShape.csgSdf (x : ℝ^n) : CompoundShape n → ℝ
  | .basic s         => s.sdf x
  | .transform s A t =>
    s.csgSdf (WithLp.toLp 2 (A.val.transpose.mulVec (WithLp.ofLp (x - t))))
  | .union a b       => min (a.csgSdf x) (b.csgSdf x)
  | .intersect a b   => max (a.csgSdf x) (b.csgSdf x)
```
Which looks good except the `WithLp.toLp 2` but that is an anoying quirk of mathlib's API regarding matrices and `ℝ^n`.


Now we can relate `sdf` and `csgSdf`. The CSG SDF under estimates the true SDF in absolute value.
```
theorem CompoundShape.abs_csgSdf_le_sdf (s : CompoundShape n) (x : ℝ^n) :
    |s.csgSdf x| ≤ |sdf s.toSet x| := sorry
```
Let's ask Aristotele ... [ahh ups not true][proof2]. Trivially not true when `n=0` but more crucially not true when `s` is an empty set. The example it gave me was intersection of two spheres centered at `2` and `-2` with radius `1`. The `csgSdf` is nonzero but the intersection is empty and the quirk of `infDist` is that it is zero for an empty set.

Maybe using `infDist` wasn't the best idea as  there is also `infEDist` which can return infinity unlike `infDist`. I thought that dealing with the type would cause more hassle than benefit `ℝ∞`. I will try later, right now just add a condition that `s.toSet ≠ ∅`, then it is correct, right Aristotele?



## Generating C Code

At this point I thought I have everything to generate the first version of the C code. So I propted
```
❯ Can you please generate equivalent C program in a new `c` director that is equivalent to the Lean code with n=3. The `BasicShape` should be a C tagged union and `CompoundShape` should be represented as a flat array in reverse Polish notation. Ignore functions manipulating `Set (ℝ^n)` as we have no runtime representation for `Set (ℝ^n)`.
```

My thinking behind the require for "flat array in reverse polish notation" was that I do not want heap allocated tree like structure but rather a flat array representation that can be easily ported to GPU too.

The AI generated working and readable C code, [csg.h][csgh] and [csg.c][csgc], but it was a bit more complicated than expected. Part of the response was
```
  Encoding of CompoundShape.transform: since transforms change the evaluation context (not just a value), they don't
  fit pure postfix on a single value stack. The encoding is:
  OP_PUSH_TRANSFORM(A,t)   ← pushes A^T·(cur−t) onto the point stack
    [sub-expression...]    ← evaluated with the new point on top
  OP_POP_TRANSFORM         ← restores the previous point
  This mirrors the Lean semantics s.csgSdf(Aᵀ·(x−t)) exactly.
```
I did not realize that the presence of transform will prevent me from having only postfix operations. Ok want to make sure that this encoding is correct. Time for more Lean!


Prompt
```
❯ In LeanCSG/StackEval.lean, can you please define a flat representation of `CompoundShape n` that can be easily evaluated using a
  stack machine. Please provide:
  - flat representation of CompoundShape that can be evaluated using stack machine
  - equivalent of `CompoundShape.csgSdf` and all the constructors
  - equivalence between the flat representation and CompoundShape
  - state theorems that this equivalence preserves all the operations
```

... and after some time of waiting I get really cool code! In just 154 line of code we have the flat representation
```lean post
inductive Instr (n : ℕ) where
  | basic         (s : BasicShape n)
  | pushTransform (A : Matrix.orthogonalGroup (Fin n) ℝ) (t : ℝ^n)
  | popTransform
  | union
  | intersect

abbrev FlatShape (n : ℕ) := List (Instr n)
```
and the translation from `CompoundSphe` to `FlatShape`
```lean post
def CompoundShape.toFlat : CompoundShape n → FlatShape n
  | .basic s         => [.basic s]
  | .transform s A t => [.pushTransform A t] ++ s.toFlat ++ [.popTransform]
  | .union a b       => a.toFlat ++ b.toFlat ++ [.union]
  | .intersect a b   => a.toFlat ++ b.toFlat ++ [.intersect]
```
And even with the key theorem, stating that flattening compound shape and evaluating is the same as evaluating `csgSdf`, and fully proven:
```
theorem FlatShape.eval_toFlat (cs : CompoundShape n) (x : ℝ^n) :
    cs.toFlat.eval x = some (cs.csgSdf x) := ...
```
you can see the full code [here][proof3].

Ok now, `FlatShape n` is more of a data structure I have in mind I would like to see in the C code. So I asked
```
❯ Can you please update the C code to mirror the Lean code in LeanCSG/StackEval.lean very closely. The Lean FlatShape should be the C CompoundShape.
```
and it did produce very nice C code that look very similar to the Lean code. Updated [csg.h][csgh2] and [csg.c][csgc2].

The main thing one has to be careful is memory management as there is no way to express it in Lean, so there is nothing for the AI to follow closely. Right now, the generated code just allocates large enough buffers on the stack. For my application this is good enough for now.


That would be all for now, let's just validate that the program is doing what we think it is doing.
```
❯ Can you please write a simple program that will generate a simple picture visualizing the computed SDF? Visualize negative numbers in a blue gradient(blue at zero -> light blue as values decrease) and orange for positive numbers (red at zero -> orange as values increase). Then create a simple test shape doing couple of transforms, unions and intersections. The picture should be a slice through XY plane.
```
And here is the picture, and it matches the instructions
![genrated_sdf](static/imgs/sdf.png)


# Summary

I would like to gather my thoughts on if this is actually feasible workflow or not. I must say I'm quite surprised how smooth it was. At the beginning, I was a bit dissapointed as the AI failed prove the theorem
```
theorem BasicShape.sdf_eq [Nontrivial (ℝ^n)] (s : BasicShape n) (x : ℝ^n) :
    _root_.sdf s.toSet x = s.sdf x := sorrry
```
and
```
theorem CompoundShape.abs_csgSdf_le_sdf (s : CompoundShape n) (x : ℝ^n) :
    |s.csgSdf x| ≤ |sdf s.toSet x| := sorry
```
and Harmonic's Aristotele is still running, but from the progress bar it seems stuck so the statement might not be correct as there is some odd edge case I do not realize making the statement incorrect.

However after that it was smooth sailing. The AI generated C code on the first try, but I realized that the code is too compilicated and I didn't want to check if it is doing the right thing. So I asked it to mirror the desirable stack based C implementation in Lean and prove it equivalent to the straight forward recursive implementation. Done in a single shot! Updating the C code, one shot again, and because it has the Lean code as a precise template there is very little room for the AI to stray off course.

The only thing one should be careful is memory management as that is not explicit in Lean. The current C implementation effectively fixes the maximum number of nodes in the CSG tree. This way everything is stack allocated and there is no heap allocation. This works for my use case but might not work in general. Further experiments are necessary to see if this will cause issues.

So overall I'm super happy with this. There are tons of other things I want to do with SDFs



[proof1]: https://live.lean-lang.org/#project=mathlib-v4.24.0&codez=

[proof2]: https://live.lean-lang.org/#project=mathlib-v4.24.0&codez=


[csgh]: https://github.com/lecopivo/LeanCSG/blob/97660e7ab097d6d4a6d2371fbe441fb7e77f9e5c/c/csg.h
[csgc]: https://github.com/lecopivo/LeanCSG/blob/97660e7ab097d6d4a6d2371fbe441fb7e77f9e5c/c/csg.c
[proof3]: https://github.com/lecopivo/LeanCSG/blob/7055f3f43e07eabebb9331cdaa0b4d16828cad28/LeanCSG/StackEval.lean#L165
[csgh2]: https://github.com/lecopivo/LeanCSG/blob/677d17747706caf308ca3fcb9055b12b22bd6602/c/csg.h
[csgc2]: https://github.com/lecopivo/LeanCSG/blob/677d17747706caf308ca3fcb9055b12b22bd6602/c/csg.c
